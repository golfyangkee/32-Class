...
class EmployeeDB:  # db 인 스키마는 사용자로 전달받아 생성 | 테이블은 employees로 클래스의 create_table(self)에서 만들겠다.
    def __init__(self, db_name):    # 입력받은 db를 생성
       self.conn  = sqlite3.connect(db_name)
       self.cursor   =self. conn.cursor()  # 명령 수행하는 커서 생성
       self.create_table()  # 테이블 생성 user함수
