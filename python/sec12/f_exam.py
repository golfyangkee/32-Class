...
@dataclass # 생성자 소멸자 겟셋 이런거 다 안 만들어도 좋다.
class Employee:
    id : int
    name:str
    age :int
...
class EmployeeDB:
     def __init__(self, db_name):  # 매개변수가 엄청 많을 수 있으니 매개변수를 클래스로 만들고 싶은 거다. VO라고 한다
...
#4.  insert_t, select, delete_t, update_t
    def  insert_employees(self,employee : Employee):
        self.cursor.execute('insert into employees ( name, age,city) values ( %s,%d,%s)'
